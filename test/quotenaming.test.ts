...
function mount() {
  ...
  h.db.exec(`INSERT INTO clients (id,ref,kind,full_name,status,created_at,updated_at)
             VALUES ('cl1','CL-0001','individual','Elena TOROPOVA','active','${AT}','${AT}'),
...
describe('what a quotation is called', () => {
  it('composes the name from the type and the client, type first', () => {
    ...
    expect(quoteName('RV. Partner', null, 'Elena TOROPOVA')).toBe('RV. Partner — Elena TOROPOVA');
  });

  it('joins the extra words to the type, not to the person', () => {
    // "VV. Parent Grandparent — Elena TOROPOVA", not
    // "VV. Parent — Elena TOROPOVA Grandparent". The extra words say what the
    // work is, so they must stay on the sorting side of the dash.
    expect(quoteName('VV. Parent', 'Grandparent', 'Elena TOROPOVA'))
      .toBe('VV. Parent Grandparent — Elena TOROPOVA');
  });

  it('reads the type through the vocabulary', () => {
    expect(quoteNameFrom(TYPES, 'rv_partner', null, 'Elena TOROPOVA'))
      .toBe('RV. Partner — Elena TOROPOVA');
  });
...
describe('creating a quotation', () => {
  ...
  it('adds the extra words where they are given', async () => {
    ...
    expect(h.get<{ description: string }>('SELECT description FROM quotes')!.description)
      .toBe('VV. Parent Grandparent second application — Elena TOROPOVA');
  });
  ...
  it('does not print a Scope paragraph on the quotation', async () => {
    ...
    expect(body).toContain('RV. Partner — Elena TOROPOVA');
