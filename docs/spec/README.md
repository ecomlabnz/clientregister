...
| [`invariants.md`](invariants.md) | The 133 things the database refuses to do, in its own words, and the 13 uniqueness rules | `npm run spec`, from the built schema |
| [`data-model.md`](data-model.md) | 53 tables, every column | `npm run spec`, from the built schema |
| [`routes.md`](routes.md) | 224 routes and the permission each runs behind, the public surface listed first | `npm run spec`, from the built router |
| [`settings.md`](settings.md) | 89 settings — everything an administrator can change without a deployment | `npm run spec`, from the modules |
