...
| [`features.md`](features.md) | 24 modules — what each is, where it is mounted, every route it owns, its settings and its place in the menu | `npm run spec`, from the registry |
| [`invariants.md`](invariants.md) | The 118 things the database refuses to do, in its own words, and the 12 uniqueness rules | `npm run spec`, from the built schema |
| [`data-model.md`](data-model.md) | 50 tables, every column | `npm run spec`, from the built schema |
| [`routes.md`](routes.md) | 217 routes and the permission each runs behind, the public surface listed first | `npm run spec`, from the built router |
| [`settings.md`](settings.md) | 88 settings — everything an administrator can change without a deployment | `npm run spec`, from the modules |
...
**2. [`invariants.md`](invariants.md).** The 118 rules the database enforces. This is
