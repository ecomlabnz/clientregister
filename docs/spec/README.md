...
| [`invariants.md`](invariants.md) | The 224 things the database refuses to do, in its own words, and the 13 uniqueness rules | `npm run spec`, from the built schema |
| [`data-model.md`](data-model.md) | 61 tables, every column | `npm run spec`, from the built schema |
| [`routes.md`](routes.md) | 237 routes and the permission each runs behind, the public surface listed first | `npm run spec`, from the built router |
...
**1. [`mistakes.md`](mistakes.md).** 47 faults, each with the rule that
...
**2. [`invariants.md`](invariants.md).** The 224 rules the database enforces. This is
