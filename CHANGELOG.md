...
### Added
**The register now refuses a value that is not on one of your lists, and there
is a page that tells you if any got in before today.**

On 12 September two raw codes were spotted on a page. Reading the whole register
against its own lists found 35 client files holding the *words* of a visa —
`Work Visa - Accredited Employer Work Visa` — where every other file holds the
short code, `wv_aewv`. They came in with the bulk load on 1 September.

Nothing looked wrong, which is the bad part. The page showed the words. But the
dropdown on the edit form only offers the short codes, so those 35 files showed
**blank** there — and saving one would have wiped the client's visa type without
saying anything.

Checking then found the real problem: **nothing anywhere was checking.** Not the
client form, not the intake assistant, not the document reader, not a bulk load.
The dropdown offered the right answers; nothing refused a wrong one.

So the check now sits in the database, which is the one place everything goes
through. Sixteen fields are covered — the visa a client holds, their English
test, title, gender and relationship status, the kind of a matter, of a
quotation, of a line on a quotation, of a warning, of a file note, of a document
heading, of a period of work, an education level and whether the course was
finished, and why and how a trip was made.

Four things about it that matter in practice:

- **Your lists are still yours.** The check reads the practice's own lists under
  Settings → Lists and dropdowns. Add a visa type there and it can be stored a
  second later, with nothing deployed.
- **Taking a word off a list does not strand the files already using it.** The
  check only applies when the value is being *changed*. A file recorded last
  year under a type since retired opens, edits and saves exactly as before.
- **Blank is always allowed.** "Not recorded" is a real answer everywhere.
- **The 35 files are untouched.** This release adds a rule; it does not rewrite
  anybody's file. They are listed on the new page, and they still save.

**Settings → Self-check** is the new page. It reads every stored value against
the list it should have come from and says what does not match, with a count.
Run it after loading data from anywhere else — load, look, fix, then rely on it.
It reports and never changes anything.

### Fixed
**Ten invented codes in the test suite's own fixtures.** The new rule found
them: `rv_partner` where the list says `rv_partnership`, `other_other` where it
says `ot_other`, `ielts_general` where it says `ielts`, and a residence *visa*
code used as a *matter* type. None was in the practice's register.

**The specification was about to stop quoting a third of what the database
refuses.** `docs/spec/invariants.md` is generated by matching the refusal text
in each trigger, and the pattern only matched a refusal that is one fixed
sentence. The 32 new ones name the value that was refused, so all 32 would have
been left out in silence. The pattern was in three files as three copies; it is
now in one, `scripts/spec-refusals.mjs`, read by all three.
